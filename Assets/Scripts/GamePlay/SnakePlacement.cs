...
namespace GamePlay
{
    ...
    public class SnakePlacement : Grid
    {
        ...
        private void PlaceSnake()
        {
            ...
            GameManager.Instance().occupiedCells.Add(snakeHeadPosition);
            ...
            for (int i = 1; i < snakeLength; i++)
            {
                ...
                GameManager.Instance().occupiedCells.Add(snakeBodyPosition);
            }
        ...
        private void FixedUpdate()
        {
            ...
            if (_timer >= movementSpeed)
            {
                ...
                Vector2Int newHeadPosition = GameManager.Instance().occupiedCells[0] + _currentDirection;

                if (IsCellWithinGrid(newHeadPosition) && !IsCellOccupied(newHeadPosition))
                {
                    //move the snake head to the new position
                    GameManager.Instance().occupiedCells.Insert(0, newHeadPosition);
                    GameManager.Instance().occupiedCells.RemoveAt(GameManager.Instance().occupiedCells.Count - 1);
                    ...
                    for (int i = 1; i < snakeLength; i++)
                    {
                        Vector3 snakeBody = new Vector3(GameManager.Instance().occupiedCells[i].x * GridCellSize, 0.5f,
                            GameManager.Instance().occupiedCells[i].y * GridCellSize);
        ...
        private void IncreaseSnakeLength()
        {
            ...
            Vector2Int lastCell = GameManager.Instance().occupiedCells[GameManager.Instance().occupiedCells.Count - 1];
            Vector2Int newBodySegmentPosition = lastCell;

            GameManager.Instance().occupiedCells.Add(newBodySegmentPosition);
